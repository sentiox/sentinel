...
check_system() {
    ...
    if [ "$openwrt_version" = "23" ]; then
        msg "OpenWrt 23.05 не поддерживается начиная с sentinel 0.5.0"
        msg "Для OpenWrt 23.05 используйте sentinel версии 0.4.11 или устанавливайте зависимости и sentinel вручную"
        msg "Подробности: https://sentinel.net/docs/install/#%d1%83%d1%81%d1%82%d0%b0%d0%bd%d0%be%d0%b2%d0%ba%d0%b0-%d0%bd%d0%b0-2305"
        exit 1
    fi

...
# Get url fragment (everything after the first '#')
url_get_fragment() {
    local url="$1"
    case "$url" in
        *"#"*) echo "${url#*#}" ;;
...
get_device_hwid() {
    ...
    echo "device-$(echo "$mac" | md5sum | cut -c1-32)"
}
...
download_to_file() {
    ...
    for attempt in $(seq 1 "$retries"); do
        ...
        if [ -n "$user_agent" ] && [ -n "$hwid" ] && command -v curl >/dev/null 2>&1; then
            # Use curl for subscription with HWID headers (busybox wget doesn't support --header)
            local proxy_opts=""
            ...
            if [ -n "$http_proxy_address" ]; then
                http_proxy="http://$http_proxy_address" https_proxy="http://$http_proxy_address" \
                    wget -U "$user_agent" -O "$filepath" "$url" && result=0
            else
                wget -U "$user_agent" -O "$filepath" "$url" && result=0
            fi
        else

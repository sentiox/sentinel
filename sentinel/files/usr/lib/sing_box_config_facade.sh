SENTINEL_LIB="/usr/lib/sentinel"
. "$SENTINEL_LIB/helpers.sh"
. "$SENTINEL_LIB/sing_box_config_manager.sh"
...
sing_box_cf_add_proxy_outbound() {
    ...
    url=$(url_decode "$url")
    local url_fragment
    url_fragment=$(url_get_fragment "$url")
    ...
    socks4 | socks4a | socks5)
        ...
        tag=$([ -n "$url_fragment" ] && echo "$url_fragment" || get_outbound_tag_by_section "$section")
...
_add_outbound_security() {
    ...
    tls | reality)
        ...
        if [ -z "$fingerprint" ] && [ "$security" = "reality" ]; then
            fingerprint="chrome"
        fi

...
'require uci';
'require fs';
'require view.sentinel.constants as constants';
'require view.sentinel.utils as utils';
...
function checkDNSAvailability() {
    return new Promise(async (resolve) => {
        try {
            safeExec('/usr/bin/sentinel', ['check_dns_available'], 'P0_PRIORITY', dnsStatusResult => {
...
function showConfigModal(command, title) {
    ...
    try {
        ...
        if (command === 'global_check') {
            safeExec('/usr/bin/sentinel', [command], 'P0_PRIORITY', res => {
...
const ButtonFactory = {
    ...
    createActionButton: function (config) {
        return this.createButton({
            ...
            onClick: () => safeExec('/usr/bin/sentinel', [config.action], 'P0_PRIORITY')
    ...
    createInitActionButton: function (config) {
        return this.createButton({
            label: config.label,
            additionalClass: `cbi-button-${config.type || ''}`,
            onClick: () => safeExec('/etc/init.d/sentinel', [config.action], 'P0_PRIORITY')
                .then(() => config.reload && location.reload()),
            style: config.style
        });
    },
...
let createStatusSection = async function () {
    // Get initial sentinel status
    let initialsentinelStatus = { enabled: false };
    try {
        const result = await fs.exec('/usr/bin/sentinel', ['get_status']);
        if (result && result.stdout) {
            const status = JSON.parse(result.stdout);
            initialsentinelStatus.enabled = status.enabled === 1;
        }
    } catch (e) {
        console.error('Error getting initial sentinel status:', e);
    }

    return E('div', { 'class': 'cbi-section' }, [
        E('div', { 'class': 'table', style: 'display: flex; gap: 20px;' }, [
            // sentinel Status Panel
            E('div', { 'id': 'sentinel-status-panel', 'class': 'panel', 'style': 'flex: 1; padding: 15px;' }, [
                E('div', { 'class': 'panel-heading' }, [
                    E('strong', {}, _('sentinel Status')),
                    E('br'),
                    E('span', { 'id': 'sentinel-status-text' }, createLoadingStatusText())
                ]),
                E('div', { 'class': 'panel-body', 'style': 'display: flex; flex-direction: column; gap: 8px;' }, [
                    ...
                    ButtonFactory.createInitActionButton({
                        label: initialsentinelStatus.enabled ? 'Disable Autostart' : 'Enable Autostart',
                        type: initialsentinelStatus.enabled ? 'remove' : 'apply',
                        action: initialsentinelStatus.enabled ? 'disable' : 'enable',
                        reload: true
                    }),
            ...
            E('div', { 'id': 'version-info-panel', 'class': 'panel', 'style': 'flex: 1; padding: 15px;' }, [
                ...
                E('div', { 'class': 'panel-body' }, [
                    E('div', { 'style': 'margin-top: 10px; font-family: monospace; white-space: pre-wrap;' }, [
                        E('strong', {}, _('sentinel: ')), E('span', { 'id': 'sentinel-version' }, _('Loading...')), '\n',
...
async function updateDiagnostics() {
    ...
    safeExec('/usr/bin/sentinel', ['get_status'], 'P0_PRIORITY', result => {
        try {
            const parsedsentinelStatus = JSON.parse(result.stdout || '{"enabled":0,"status":"error"}');

            // Update sentinel status text
            updateTextElement('sentinel-status-text',
                E('span', {
                    'style': `color: ${parsedsentinelStatus.enabled ? constants.STATUS_COLORS.SUCCESS : constants.STATUS_COLORS.ERROR}`
                }, [
                    parsedsentinelStatus.enabled ? '✔ Autostart enabled' : '✘ Autostart disabled'
                ])
            );

            // Update autostart button
            const autostartButton = parsedsentinelStatus.enabled ?
                ButtonFactory.createInitActionButton({
            ...
            const panel = document.getElementById('sentinel-status-panel');
            if (panel) {
    ...
    safeExec('/usr/bin/sentinel', ['get_sing_box_status'], 'P0_PRIORITY', result => {
        try {
    ...
    safeExec('/usr/bin/sentinel', ['show_version'], 'P2_PRIORITY', result => {
        updateTextElement('sentinel-version',
            document.createTextNode(result.stdout ? result.stdout.trim() : _('Unknown'))
        );
    });

    safeExec('/usr/bin/sentinel', ['show_luci_version'], 'P2_PRIORITY', result => {
        updateTextElement('luci-version',
            document.createTextNode(result.stdout ? result.stdout.trim() : _('Unknown'))
        );
    });

    safeExec('/usr/bin/sentinel', ['show_sing_box_version'], 'P2_PRIORITY', result => {
        updateTextElement('singbox-version',
            document.createTextNode(result.stdout ? result.stdout.trim() : _('Unknown'))
        );
    });

    safeExec('/usr/bin/sentinel', ['show_system_info'], 'P2_PRIORITY', result => {
        if (result.stdout) {
    ...
    runAsyncTask(async () => {
        try {
            ...
            const data = await uci.load('sentinel');
            const proxyString = uci.get('sentinel', 'main', 'proxy_string');
...
function setupDiagnosticsEventHandlers(node) {
    const titleDiv = E('h2', { 'class': 'cbi-map-title' }, _('sentinel'));
    ...
    document.addEventListener('visibilitychange', function () {
        ...
        if (document.hidden || !diagnosticsTab || !diagnosticsTab.classList.contains('cbi-tab-active')) {
            ...
            // Don't stop error polling here - it's managed in sentinel.js for all tabs

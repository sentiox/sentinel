...
// src/sentinel/methods/custom/getConfigSections.ts
async function getConfigSections() {
  return uci.load("sentinel").then(() => uci.sections("sentinel"));
}

// src/sentinel/methods/shell/callBaseMethod.ts
async function callBaseMethod(method, args = [], command = "/usr/bin/sentinel") {
...
((Sentinel2) => {
  ...
  })(AvailableClashAPIMethods = Sentinel2.AvailableClashAPIMethods || (Sentinel2.AvailableClashAPIMethods = {}));
})(Sentinel || (Sentinel = {}));

// src/sentinel/methods/shell/index.ts
var SentinelShellMethods = {
  ...
  getStatus: async () => callBaseMethod(Sentinel.AvailableMethods.GET_STATUS),
  ...
  getClashApiProxies: async () => callBaseMethod(Sentinel.AvailableMethods.CLASH_API, [
  ...
  setClashApiGroupProxy: async (group, proxy) => callBaseMethod(Sentinel.AvailableMethods.CLASH_API, [
    Sentinel.AvailableClashAPIMethods.SET_GROUP_PROXY,
    group,
    proxy
  ]),
  subscriptionUpdate: async () => callBaseMethod(
    Sentinel.AvailableMethods.SUBSCRIPTION_UPDATE
  ),
...
// src/sentinel/methods/custom/getDashboardSections.ts
async function getFailoverState(sectionName) {
  try {
    const content = await fs.read("/tmp/sentinel_failover_state_" + sectionName);
    if (!content || !content.trim()) return null;
    const lines = content.trim().split("\n");
    if (lines.length < 2) return null;
    const originalProxy = lines[0];
    const failoverTime = parseInt(lines[1], 10);
    const now = Math.floor(Date.now() / 1000);
    const elapsed = now - failoverTime;
    const remainingMin = Math.max(0, Math.floor((18000 - elapsed) / 60));
    return { originalProxy, failoverTime, remainingMin };
  } catch (e) {
    return null;
  }
}
async function getDashboardSections() {
  ...
  await Promise.all(filteredSections.map(async (section) => {
    const state = await getFailoverState(section[".name"]);
    if (state) failoverStates[section[".name"]] = state;
  }));
  const data = filteredSections.map((section) => {
    if (section.connection_type === "proxy") {
      ...
      if (section.proxy_config_type === "selector") {
        ...
        return {
          withTagSelect: true,
          failoverState: failoverStates[section[".name"]] || null,
...
// src/sentinel/methods/custom/getClashApiSecret.ts
...
// src/sentinel/methods/custom/index.ts
var CustomSentinelMethods = {
  getConfigSections,
...
var SENTINEL_LUCI_APP_VERSION = "__COMPILED_VERSION_VARIABLE__";
...
// src/sentinel/methods/fakeip/getFakeIpCheck.ts
...
// src/sentinel/methods/fakeip/getIpCheck.ts
...
// src/sentinel/methods/fakeip/index.ts
var RemoteFakeIPMethods = {
  getFakeIpCheck,
  getIpCheck
};

// src/sentinel/services/tab.service.ts
...
// src/sentinel/tabs/diagnostic/helpers/getCheckTitle.ts
function getCheckTitle(name) {
  return `${name} ${_("checks")}`;
}

// src/sentinel/tabs/diagnostic/checks/contstants.ts
...
// src/sentinel/tabs/diagnostic/diagnostic.store.ts
...
// src/sentinel/services/store.service.ts
...
// src/sentinel/services/logger.service.ts
var Logger = class {
  constructor() {
    this.logs = [];
...
// src/sentinel/services/sentinelLogWatcher.service.ts
var SentinelLogWatcher = class _SentinelLogWatcher {
  constructor() {
    this.intervalMs = 5e3;
  ...
  static getInstance() {
    if (!_SentinelLogWatcher.instance) {
      _SentinelLogWatcher.instance = new _SentinelLogWatcher();
    }
    return _SentinelLogWatcher.instance;
  }
...
// src/sentinel/services/core.service.ts
...
// src/sentinel/services/socket.service.ts
...
// src/sentinel/tabs/dashboard/partials/renderSections.ts
...
function renderFailoverBadge(failoverState) {
  if (!failoverState) return null;
  const { originalProxy, remainingMin } = failoverState;
  const hours = Math.floor(remainingMin / 60);
  const mins = remainingMin % 60;
  const timeStr = hours > 0 ? `${hours}h ${mins}m` : `${mins}m`;
  return E("div", {
    style: "background: #e67e22; color: #fff; padding: 4px 10px; border-radius: 4px; font-size: 12px; margin-bottom: 6px;"
  }, `\u26A0 Failover: \u0436\u0434\u0443 ${originalProxy} (${timeStr})`);
}
function renderDefaultState({
  ...
  const failoverBadge = section.failoverState ? renderFailoverBadge(section.failoverState) : undefined;
  const sectionChildren = [];
  if (failoverBadge) sectionChildren.push(failoverBadge);
  sectionChildren.push(
    ...
    E("div", { class: "pdk_dashboard-page__outbound-section__title-section" }, [
      ...
      E("div", { style: "display: flex; gap: 8px; align-items: center" }, [
        onSubscriptionUpdate ? (
          subscriptionUpdating ? E("div", { class: "skeleton", style: "width: 140px; height: 28px" }) : E(
            "button",
            {
  ...
  return E("div", { class: "pdk_dashboard-page__outbound-section" }, sectionChildren);
}
...
// src/sentinel/tabs/dashboard/partials/renderWidget.ts
...
// src/sentinel/tabs/dashboard/render.ts
function render() {
  return E(
    "div",
    {
...
// src/sentinel/fetchers/fetchServicesInfo.ts
...
// src/sentinel/tabs/dashboard/initController.ts
...
var subscriptionUpdating = false;
async function handleSubscriptionUpdate() {
  subscriptionUpdating = true;
  await renderSectionsWidget();
  await SentinelShellMethods.subscriptionUpdate();
...
async function renderSectionsWidget() {
  ...
  const renderedWidgets = sectionsWidget.data.map(
    (section) => renderSections({
      ...
      section,
      latencyFetching: sectionsWidget.latencyFetching,
      onSubscriptionUpdate: section.isSubscription ? handleSubscriptionUpdate : null,
      subscriptionUpdating,
      onTestLatency: (tag) => {
        if (section.withTagSelect) {
...
async function renderServicesInfoWidget() {
  ...
  const renderedWidget = renderWidget({
    ...
    items: [
      {
        key: _("Sentinel"),
        value: servicesInfoWidget.data.sentinel ? _("\u2714 Enabled") : _("\u2718 Disabled"),
        attributes: {
          class: servicesInfoWidget.data.sentinel ? "pdk_dashboard-page__widgets-section__item__row--success" : "pdk_dashboard-page__widgets-section__item__row--error"
        }
      },
      {
        key: _("Sing-box"),
...
// src/sentinel/tabs/dashboard/styles.ts
var styles = `
...
// src/sentinel/tabs/dashboard/index.ts
var DashboardTab = {
  render,
  initController,
  styles
...
// src/sentinel/tabs/diagnostic/renderDiagnostic.ts
function render2() {
...
// src/sentinel/tabs/diagnostic/checks/updateCheckStore.ts
...
// src/sentinel/tabs/diagnostic/helpers/getMeta.ts
...
// src/sentinel/tabs/diagnostic/checks/runDnsCheck.ts
...
// src/sentinel/tabs/diagnostic/checks/runSingBoxCheck.ts
...
// src/sentinel/tabs/diagnostic/checks/runNftCheck.ts
...
// src/sentinel/tabs/diagnostic/checks/runFakeIPCheck.ts
...
// src/sentinel/tabs/diagnostic/partials/renderAvailableActions.ts
...
// src/sentinel/tabs/diagnostic/partials/renderCheckSection.ts
...
// src/sentinel/tabs/diagnostic/partials/renderRunAction.ts
...
// src/sentinel/tabs/diagnostic/partials/renderSystemInfo.ts
...
// src/sentinel/tabs/diagnostic/partials/renderWikiDisclaimer.ts
...
// src/sentinel/tabs/diagnostic/checks/runSectionsCheck.ts
...
// src/sentinel/tabs/diagnostic/initController.ts
...
// src/sentinel/tabs/diagnostic/styles.ts
var styles4 = `
...
// src/sentinel/tabs/diagnostic/index.ts
var DiagnosticTab = {
  render: render2,
...
return baseclass.extend({
  ...
  IP_CHECK_DOMAIN,
  Logger,
  SENTINEL_LUCI_APP_VERSION,
  SentinelShellMethods,
  REGIONAL_OPTIONS,
  RemoteFakeIPMethods,
  STATUS_COLORS,
  TabService,

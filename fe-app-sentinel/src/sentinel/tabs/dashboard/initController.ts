...
import { CustomSentinelMethods, SentinelShellMethods } from '../../methods';
...
async function fetchDashboardSections() {
  ...
  const { data, success } = await CustomSentinelMethods.getDashboardSections();
...
async function handleChooseOutbound(selector: string, tag: string) {
  await SentinelShellMethods.setClashApiGroupProxy(selector, tag);
...
async function handleTestGroupLatency(tag: string) {
  ...
  await SentinelShellMethods.getClashApiGroupLatency(tag);
  await fetchDashboardSections();
...
async function handleTestProxyLatency(tag: string) {
  ...
  await SentinelShellMethods.getClashApiProxyLatency(tag);
  await fetchDashboardSections();
    ...
    items: [
      {
        ...
        value: servicesInfoWidget.data.sentinel
          ...
          : _('✘ Disabled'),
        attributes: {
          class: servicesInfoWidget.data.sentinel

...
import { SENTINEL_LUCI_APP_VERSION } from '../../../constants';
...
async function fetchSystemInfo() {
  const systemInfo = await SentinelShellMethods.getSystemInfo();
  ...
  if (systemInfo.success) {
    ...
    store.set({
      diagnosticsSystemInfo: {
        loading: false,
        sentinel_version: _('unknown'),
        sentinel_latest_version: _('unknown'),
...
async function handleRestart() {
  ...
  try {
    await SentinelShellMethods.restart();
  } catch (e) {
...
async function handleStop() {
  ...
  try {
    await SentinelShellMethods.stop();
  } catch (e) {
...
async function handleStart() {
  ...
  try {
    await SentinelShellMethods.start();
  } catch (e) {
...
async function handleEnable() {
  ...
  try {
    await SentinelShellMethods.enable();
  } catch (e) {
...
async function handleDisable() {
  ...
  try {
    await SentinelShellMethods.disable();
  } catch (e) {
...
async function handleShowGlobalCheck() {
  ...
  try {
    const globalCheck = await SentinelShellMethods.globalCheck();
...
async function handleViewLogs() {
  ...
  try {
    const viewLogs = await SentinelShellMethods.checkLogs();
...
async function handleShowSingBoxConfig() {
  ...
  try {
    const showSingBoxConfig = await SentinelShellMethods.showSingBoxConfig();
...
function renderDiagnosticAvailableActionsWidget() {
  ...
  const sentinelEnabled = Boolean(servicesInfoWidget.data.sentinel);
...
function renderDiagnosticSystemInfoWidget() {
  ...
  function getSentinelVersionRow(): IRenderSystemInfoRow {
    ...
    const hasActualVersion =
      Boolean(diagnosticsSystemInfo.sentinel_latest_version) &&
      diagnosticsSystemInfo.sentinel_latest_version !== 'unknown';
  ...
  const renderedSystemInfo = renderSystemInfo({
    items: [
      getSentinelVersionRow(),
      {
        key: 'Luci App',
        value: normalizeCompiledVersion(SENTINEL_LUCI_APP_VERSION),
      },
      {
        key: 'Sing-box',
